#ifndef LEVENBERG_H_INCLUDED
#define LEVENBERG_H_INCLUDED
...
template<class real>
class levenberg : public base_method<real> {
public:
    levenberg() : base_method<real>(){}
    levenberg(real epsilon) : base_method<real>(epsilon){}
    levenberg(real epsilon, size_t max_iter) : base_method<real>(epsilon, max_iter) {}
    levenberg(real epsilon, size_t max_iter, real working_precision) : base_method<real>(epsilon, max_iter, working_precision) {}
    ...
    void operator()(function::function<real>& f, line_search::base_line_search<real>& ls, arma::Col<real>& x) {
        ...
        while (arma::norm(gr) > this->epsilon && this->iter_count < this->max_iter &&
            ...
            auto leftTerm = hes + (lambda * arma::eye(n, n));
